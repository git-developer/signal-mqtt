...
attachments, quotations and emojis to/from Signal accounts and groups.
...
services:
  signal-mqtt:
    image: ghcr.io/git-developer/signal-mqtt
...
_<PREFIX>_ `/method/` _<METHOD>_ [`:` _<PAYLOAD_NAME>_ ] [`/` _<PARAMETER_NAME>_ `/` _<PARAMETER_VALUE>_ ]…
...
* An optional name for the payload parameter, defaulting to `message`
...
====== Send an attachment
```sh
$ mosquitto_pub -h broker -t signal/out/method/send:attachment/recipient/%2B491713920000 -m 'data:image/png;base64,iVBORw0K…'
```
The BASE64 encoded image attachment is sent to the phone.
...
|`MQTT_LOG_TOPIC`|MQTT topic to publish the log to|http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/os/mqtt-v3.1.1-os.html#_Toc398718106[Topic names]|`${MQTT_TOPIC_PREFIX}/log`|`chats/logs`
